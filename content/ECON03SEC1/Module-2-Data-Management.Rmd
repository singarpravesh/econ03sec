...
# Contents
[2.1 Data transformation](#2.1) <br>
[2.2 Importing and editing data from text files/documents](#2.2)<br>
[2.3 Creating named ranges](#2.3)<br>
[2.4 `IF()` statement (Part - 1)](#2.4)<br>
[2.5 The `COUNTIF()`, `COUNTIFS()`, `COUNT()`, `COUNTA()`, and `COUNTBLANK()` functions](#2.5)<br>
[2.6 The `SUMIF()`, `AVERAGEIF()`, `SUMIFS()`, and `AVERAGEIFS()` functions](#2.6)<br>
[2.7 Sorting](#2.7)<br>
[2.8 Filtering and removing duplicates](#2.8)<br>
[2.9 Lookup functions](#2.9)

---

## 2.1 Data transformation {#2.1}
...
## 2.2 Importing and editing data from text files/documents.{#2.2}
...
## 2.5 The `COUNTIF()`, `COUNTIFS()`, `COUNT()`, `COUNTA()`, and `COUNTBLANK()` functions.{#2.5}
...
- Download the file [Rock.xlsx](Rock.xlsx) used in the video and practice.
- Exercises:
  - The following questions refer to the [Rock.xlsx](Rock.xlsx) file:
...
## 2.6 The `SUMIF()`, `AVERAGEIF()`, `SUMIFS()`, and `AVERAGEIFS()` functions {#2.6}
...
- Download the file [Makeup2007.xlsx](Makeup2007.xlsx) used in the videos and practice.
- Exercises:
  - The following questions refer to the [Makeup2007.xlsx](Makeup2007.xlsx) file.
  ...
  7. The [NBA.xlsx](NBA.xlsx) file contains the following information:
  ...
  8. The [Toysrus.xlsx](Toysrus.xlsx) file contains sales revenue (in millions of dollars) during each quarter for the years 1997–2001 and the first two quarters of 2002. Use this data to compute a seasonal index for each quarter of the year. For example, if average sales during the first quarter were 80 percent of the overall average sales per quarter, the first quarter would have a seasonal index of 0.8.
  9. The [Sumifrows.xlsx](Sumifrows.xlsx) file contains sales data during several winter, spring, summer, and fall quarters. Determine average sales during the winter, spring, summer, and fall quarters.
  10. Again, by using the [Makeup2007.xlsx](Makeup2007.xlsx) file, determine how much revenue was made on sales transactions involving at least 50 units of makeup.
  ...
  17. From the [Numbers.xlsx](Numbers.xlsx) file, find the total of all numbers between 5 and 15, inclusive.
  18. From the [Numbers.xlsx](Numbers.xlsx) file, find the average of all numbers between 10 and 25, inclusive.
...
- Download the file [Makeupsorttemp.xlsx](Makeupsorttemp.xlsx) used in the videos and practice.
- Exercises:
  1. In the [Makeupsort.xlsx](Makeupsort.xlsx) file, sort the sales data alphabetically by location and then by product type, name, date of sale, and units sold.
  2. The [Sortday.xlsx](Sortday.xlsx) file contains hours worked on different days of the week. Sort the data so that Monday data is followed by Tuesday data and so on.
  3. The [Sorticons.xlsx](Sorticons.xlsx) file contains annual investment returns with an up arrow used to indicate good years, a horizontal arrow used to indicate average years, and a red down arrow used to indicate bad years. Sort the data by the icons in the Stock column with up arrows on top, followed by horizontal arrows and then red down arrows.
  4. The [Makeupsortfont.xlsx](Makeupsortfont.xlsx) file contains your makeup data with certain dates shown in blue, red, or brown font. Sort the data so that brown dates are on top, followed by red dates and then blue dates

---

## 2.8 Filtering and removing duplicates {#2.8}
...
- Download and use the [Makeupfilter.xlsxs](Makeupfilter.xlsxs) file to practice.
- Exercises:
  - The following questions relate the [Makeupfilter.xlsxs](Makeupfilter.xlsxs) file.
